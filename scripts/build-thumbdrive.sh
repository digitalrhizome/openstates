...
if ! [ -z "$(ls -A .)" ]; then
	...
	echo "Do you wish to delete any existing Open States directories on this drive? (y or n)"
	read -p "> " wipedrive
	if [ "$wipedrive" != "y" ]; then
...
mkdir docker-images
images="openstates/scrape openstates/openstates openstates.org mongo mysql"
...
Now you can run \`docker-compose build openstates\` in your Open States directory.

## Read the documentation

The documentation [is available online](https://docs.openstates.org), but is also on this drive. Simply open `./documentation/_build/html/index.html` in a web browser.
" > ./README.md
